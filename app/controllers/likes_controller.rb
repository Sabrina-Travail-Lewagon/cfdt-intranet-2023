class LikesController < ApplicationController
  ...
  def create
    ...
    authorize @like
    if @like.save
      flash[:notice] = 'Vous avez aimé cet article.'
    else
  ...
  def destroy
    ...
    authorize @like
    if @like.destroy
      flash[:notice] = 'Vous avez retiré votre j\'aime de cet article.'
    else
      # Ce message s'affiche si l'utilisateur essaie de retirer un like qui n'existe pas ou s'il y a une erreur de suppression
      flash[:alert] = 'Vous ne pouvez pas retirer un j\'aime d\'un article que vous n\'avez pas aimé ou une erreur est survenue.'
    end
    redirect_to articles_path
  end
